...
struct GridPhotosSheetView: View {
    @StateObject private var viewModel: GridPhotosSheetViewModel
    ...
    init(cell: CalendarDetailCell,
        ...
        self._viewModel = StateObject(wrappedValue: GridPhotosSheetViewModel(cell: cell))
    ...
    var body: some View {
        ...
        .sheet(item: $pagerInfo) { pager in
            CalendarPhotoPagerView(photos: pager.photos,
    ...
    private var photoGrid: some View {
        ForEach(viewModel.sortedPhotos, id: \.id) { photo in
            VStack(spacing: 2) {
            ...
            .onTapGesture {
                let photosSorted = viewModel.sortedPhotos

                if let idx = photosSorted.firstIndex(where: { $0.id == photo.id }) {
                    pagerInfo = PagerInfo(photos: photosSorted,
                                          index: idx,
