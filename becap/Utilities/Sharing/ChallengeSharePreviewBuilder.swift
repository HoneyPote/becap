...
enum ChallengeSharePreviewBuilder {
    ...
    private static func drawBackground(in context: CGContext, size: CGSize) {
        ...
        let overlay = UIBezierPath(roundedRect: CGRect(x: 60, y: 240, width: size.width - 120, height: size.height - 360), cornerRadius: 44)
    ...
    private static func drawContent(for challenge: Challenge, code: String, in context: CGContext, size: CGSize) {
        ...
        let brand = "Becap"
        (brand as NSString).draw(at: CGPoint(x: 80, y: 84), withAttributes: brandAttributes)
        ...
        draw(text: tagline,
             in: CGRect(x: 120, y: 210, width: size.width - 240, height: 160),
        ...
        draw(text: title,
             in: CGRect(x: 140, y: 380, width: size.width - 280, height: 280),
        ...
        draw(text: description,
             in: CGRect(x: 140, y: 650, width: size.width - 320, height: 220),
    ...
    private static func drawCodeBadge(code: String, in context: CGContext, size: CGSize) {
        let badgeRect = CGRect(x: 140, y: size.height - 340, width: size.width - 280, height: 160)
        let path = UIBezierPath(roundedRect: badgeRect, cornerRadius: 36)
        ...
        draw(text: "Code d'accès", in: CGRect(x: badgeRect.minX + 40, y: badgeRect.minY + 28, width: badgeRect.width - 80, height: 40), attributes: titleAttributes)

        let codeAttributes: [NSAttributedString.Key: Any] = [
            .font: UIFont.monospacedDigitSystemFont(ofSize: 60, weight: .bold),
            .foregroundColor: UIColor.white
        ]
        (code as NSString).draw(in: CGRect(x: badgeRect.minX + 40, y: badgeRect.minY + 76, width: badgeRect.width - 80, height: 80), withAttributes: codeAttributes)
    }
    ...
    private static func drawFooter(in context: CGContext, size: CGSize) {
        ...
        (footer as NSString).draw(at: CGPoint(x: 140, y: size.height - 140), withAttributes: footerAttributes)
    }
